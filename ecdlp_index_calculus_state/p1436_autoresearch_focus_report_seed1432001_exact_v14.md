...
## Next Action
1. `end_to_end_cost_reduction`: Reduce charged source generation, retained residual traffic, or matrix work without changing the frozen evidence set.
   Decisive test: Measure source generation, retained traffic, matrix work, and descent at three sizes.
   Falsifier: The fitted total exponent is not below the preregistered gate or any cell exceeds rho.
   Required artifacts: charged_costs.json, scaling_fit.json
...
1. `end_to_end_cost_reduction`: Reduce charged source generation, retained residual traffic, or matrix work without changing the frozen evidence set.
   ...
   Falsifier: The fitted total exponent is not below the preregistered gate or any cell exceeds rho.
2. `shoup_pressure_scaling_probe`: Fit charged natural-route operations versus group order and check explicit Shoup-pressure residuals.
...
## Experiment Lineage
Baseline / queued / deferred nodes: `1/2/0`; mode `logical_plan_only`.
