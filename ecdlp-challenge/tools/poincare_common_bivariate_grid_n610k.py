...
def direct_numerator(target, u_ring, u, support, matrix, x_values, parameter, level):
...
def reverse_numerator(target, field, u_ring, support, matrix, x_values, parameter, level):
...
def main():
    ...
    rows = []
    for level in (0, 1, 17):
        direct = direct_numerator(target, u_ring, u, support, matrix, x_values, parameter, level)
        reverse = reverse_numerator(target, field, u_ring, support, matrix, x_values, parameter, level)
        mismatches = []
        nonzero = 0
