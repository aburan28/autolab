#!/usr/bin/env python3
"""Compile and run a shared-denominator native kernel for fused candidates.

The compact native kernel computes each affine-add candidate independently.
This probe specializes the repeated-coordinate shape: cases with the same
field, curve, left x-coordinate, and right point share the denominator
``right_x - left_x`` and its inverse.  The generated C prepares that shared
denominator once per group, then computes each candidate point from the
row/event-specific left y-coordinate.

This is still a reference kernel, but it is the first native specialization
that directly uses the repeated-coordinate structure responsible for the 744
event-key reuse.
"""
...
DEFAULT_NATIVE_KERNEL_SOURCE = (
    DEFAULT_STATE_DIR / "ffe_public_repeated_coordinate_fused_kernel_native_kernel_target67_672_744.json"
)
DEFAULT_OUT = DEFAULT_STATE_DIR / "ffe_public_repeated_coordinate_fused_kernel_shared_denominator.json"
    ...
    for record in records:
        ...
        for group in record.get("trace_groups") or []:
            ...
            if str(trace.get("operation") or "") != "addition":
                raise ValueError("shared-denominator probe currently expects affine addition cases")
...
def grouped_cases(cases: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for group_index, key in enumerate(sorted(by_group, key=lambda item: (-len(by_group[item]), item))):
        ...
        first = group_cases[0]
        start = len(ordered_cases)
        ordered_cases.extend(group_cases)
        groups.append(
            {
    ...
    return groups, ordered_cases


def render_c_source(groups: list[dict[str, Any]], cases: list[dict[str, Any]]) -> str:
    group_entries = []
    for group in groups:
        ...
        group_entries.append(
            ...
            f"{as_int(group['left_x'])}ULL, "
            f"{as_int(group['right_x'])}ULL, "
            f"{as_int(group['right_y'])}ULL, "
            f"{as_int(group['expected_slope_denominator'])}ULL, "
            f"{as_int(group['expected_slope_denominator_inverse'])}ULL, "
            f"{as_int(group['case_start'])}, "
            f"{as_int(group['case_count'])}"
            "},"
        )
...
def run_shared_kernel(c_source: str, c_out: Path, compiler: str, keep_exe: bool) -> dict[str, Any]:
    c_out.parent.mkdir(parents=True, exist_ok=True)
    c_out.write_text(c_source)
    temp_root = Path("/private/tmp")
    exe_parent = c_out.parent if keep_exe else Path(tempfile.mkdtemp(prefix="ecdlp_shared_denom_", dir=temp_root))
...
def native_kernel_counts(native_kernel_source: dict[str, Any] | None) -> dict[str, Any]:
    if not isinstance(native_kernel_source, dict):
        return {}
    summary = native_kernel_source.get("summary") or {}
    return {
...
def source_record_summaries(records: list[dict[str, Any]], cases: list[dict[str, Any]]) -> list[dict[str, Any]]:
    source_counts: Counter[str] = Counter(case["source_name"] for case in cases)
        ...
        summaries.append(
            {
                ...
                "shared_denominator_status": "pending_global_shared_denominator_result",
                "affine_trace_status": record.get("affine_trace_status"),
                "case_count": source_counts[source_name],
...
def main() -> int:
    ...
    parser.add_argument("--native-kernel-source", type=Path, default=DEFAULT_NATIVE_KERNEL_SOURCE)
    ...
    native_kernel = load_json(args.native_kernel_source) if args.native_kernel_source.exists() else None
    ...
    groups, ordered_cases = grouped_cases(cases)
    c_source = render_c_source(groups, ordered_cases)
    c_out = args.c_out or args.out.with_suffix(".c")
    native = run_shared_kernel(c_source, c_out, args.cc, args.keep_exe)
    native_summary = native.get("native_summary") if isinstance(native.get("native_summary"), dict) else {}
    baseline = native_kernel_counts(native_kernel)
    ...
    record_summaries = source_record_summaries(records, cases)
    for summary in record_summaries:
        summary["shared_denominator_status"] = (
            "shared_denominator_native_kernel_verified" if global_ok else "shared_denominator_native_kernel_failed_check"
        )
    ...
    output = {
        "schema": "ecdlp_public_repeated_coordinate_fused_kernel_shared_denominator_probe_v1",
        "method": "compiled_c_shared_denominator_kernel_for_repeated_coordinate_candidate_points",
        "parameters": {
            "affine_source": str(args.affine_source),
            "native_kernel_source": str(args.native_kernel_source),
        ...
        "summary": {
            ...
            "shared_denominator_verified_count": len(records) if global_ok else 0,
            "below_rho_shared_denominator_verified_count": len(below) if global_ok else 0,
            "event_reuse_below_rho_shared_denominator_verified_count": len(event_reuse) if global_ok else 0,
            ...
            "baseline_native_kernel_field_op_counts": baseline_ops or None,
            "field_op_savings_vs_native_kernel": savings or None,
            "compiled": bool(native.get("compiled")),
            "executed": bool(native.get("executed")),
            "global_shared_denominator_verified": global_ok,
            "interpretation": (
                "Repeated-coordinate candidate points now share denominator "
                "and inverse preparation across native C kernel cases."
            ),
        },
